...
const schoolOptions = [
  { id: 'direito', name: 'Escola de Direito' },
  { id: 'power-skills', name: 'Escola de Power Skills' },
  { id: 'gestao-tech', name: 'Escola de Gestão, Tecnologia e Inovação Jurídica' },
];
...
const InternalCourses = () => {
  ...
  const getDynamicFilters = () => {
    ...
    // Se uma escola estiver selecionada, adicionar categorias correspondentes
    if (activeFilters.school && categoriesBySchool[activeFilters.school as keyof typeof categoriesBySchool]) {
      filters.push({
        id: 'category',
        name: 'Categoria',
        options: categoriesBySchool[activeFilters.school as keyof typeof categoriesBySchool],
      });
    }
  ...
  const handleFilterChange = (filterId: string, value: string) => {
    ...
    if (value && value !== 'all') {
      ...
      // Se mudou a escola, limpar o filtro de categoria
      if (filterId === 'school') {
    ...
    Object.entries(newFilters).forEach(([key, value]) => {
      switch (key) {
        case 'category':
          ...
          break;
        case 'school':
          filtered = filtered.filter(course => course.school === value);
          break;
      }
    });
      ...
      <main className="pt-6 md:pt-20">
        <div className="ejup-container py-4 md:py-12">
          <div className="mb-6 md:mb-12">
            <h1 className="text-2xl md:text-3xl lg:text-4xl font-bold mb-2 md:mb-4">Adquirir Novos Cursos</h1>
